...
function EditorPane({ groupId, isActive, onFocus }: EditorPaneProps) {
  ...
  const handleEditorMount: OnMount = (editor, monaco) => {
    ...
    // Setup multi-cursor shortcuts - get state at execution time
    ...
    editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyU, () => {
      const lastOp = useKyroStore.getState().popCursorOperation();
      if (lastOp) {
    ...
    editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.Backslash, () => {
      ...
      if (state.splitDirection === 'none') {
        const newGroupId = state.createEditorGroup();
        const currentActiveTab = state.editorGroups.find(g => g.id === state.activeGroupId)?.tabs[
          state.editorGroups.find(g => g.id === state.activeGroupId)?.activeTabIndex ?? -1
        ];
        if (currentActiveTab) {
          state.addTabToGroup(newGroupId, { ...currentActiveTab });
        }
