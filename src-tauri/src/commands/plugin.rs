...
pub async fn install_plugin(path: String) -> Result<PluginInfo, String> {
    ...
    let manifest = read_manifest(&dest).unwrap_or(PluginManifest {
        name: path.split('/').last().unwrap_or("plugin").to_string(),
...
pub async fn execute_plugin_function(plugin_id: String, function: String, args: Option<String>) -> Result<String, String> {
    ...
    let dir = state.plugin_dirs.get(&plugin_id).ok_or("Plugin directory not found")?;
    ...
    Err(format!("Function '{}' not found in plugin manifest", function))
}
...
pub async fn reload_plugins() -> Result<usize, String> {
    ...
    for entry in entries.filter_map(|e| e.ok()) {
        ...
        if state.plugins.contains_key(&id) { continue; }
