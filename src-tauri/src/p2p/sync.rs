...
impl DocumentSynchronizer {
    ...
    /// Apply local edit
    pub async fn apply_local_edit(&self, doc_id: &DocumentId, edit: EditOperation) -> anyhow::Result<()> {
    ...
    pub async fn update_cursor(&self, doc_id: &DocumentId, position: CursorPosition) -> anyhow::Result<()> {
    ...
    /// Update selection
    pub async fn update_selection(&self, doc_id: &DocumentId, selection: Selection) -> anyhow::Result<()> {
        let mut awareness_state = self.local_awareness.lock().await;
    ...
    pub async fn handle_remote_cursor(&self, doc_id: &DocumentId, from: PeerId, position: CursorPosition) -> anyhow::Result<()> {
        let _ = self.event_tx.send(SyncEvent::CursorUpdate {
            doc_id: doc_id.clone(),
            from,
            position,
    ...
    pub async fn handle_remote_selection(&self, doc_id: &DocumentId, from: PeerId, selection: Selection) -> anyhow::Result<()> {
        let _ = self.event_tx.send(SyncEvent::SelectionUpdate {
            doc_id: doc_id.clone(),
            from,
            selection,
